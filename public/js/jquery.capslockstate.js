...
(function($) {
	...
	var methods = {
		init : function(options) {
			...
			var helpers = {
				...
				hasStateChange : function(previousState, currentState) {
					...
					if (previousState !== currentState) {
						$('#password').trigger("capsChanged");

						if (currentState === true) {
							$('#password').trigger("capsOn");
						} else if (currentState === false) {
							$('#password').trigger("capsOff");
						} else if (currentState === "unknown") {
							$('#password').trigger("capsUnknown");
						}
					}
				}
			};
			...
			// Check all keys
			$('#password').bind("keypress.capslockstate", function(event) {
			...
			$('#password').bind("keydown.capslockstate", function(event) {
				var previousState = capsLockState;
